...
if (!empty($_GET['warid'])) {
	...
	if ($config['TFSVersion'] == 'Old_TFS02' || $config['TFSVersion'] == 'TFS_10') $war = get_guild_war($warid);
	else if ($config['TFSVersion'] == 'OTH') $war = get_guild_war03($warid);
	...
	if ($war != false) {
		...
		if ($config['TFSVersion'] == 'OTH' || $config['TFSVersion'] == 'TFS_10') $kills = get_war_kills($warid);
		...
		<ul class="war_list">
			...
			<?php if ($config['TFSVersion'] == 'OTH' || $config['TFSVersion'] == 'TFS_10') { ?>
			<li>
...
<a href=\"characterprofile.php?name=" . urlencode($death['name']) . "\"><b>".$death['name']."</b></a> ";
	...
	foreach ($wardata as $wars) {
		if ($config['TFSVersion'] == 'OTH' || $config['TFSVersion'] == 'TFS_10') $killsdata[$wars['id']] = get_war_kills($wars['id']);
