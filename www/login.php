...
if (empty($_POST) === false) {
	...
	if (empty($username) || empty($password)) {
	...
	} else if (strlen($username) > 8 || strlen($password) > 64) {
			$errors[] = 'Account Number or password is too long.';
		...
		// Starting loging
		if ($config['TFSVersion'] == 'OTH' || $config['TFSVersion'] == 'TFS_10') $login = user_login($username, $password);
